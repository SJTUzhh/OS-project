...
Task pickNextTask(){ // rear will not be used
	Task tmp1;
	struct node *tmp3 = head; // store the location of the node with the highest for the time being
	struct node *tmp2 = head; // the node that moves
	
	tmp2 = tmp2->next;
	while(tmp2 != NULL){
		if(tmp2->task.priority > tmp3->task.priority){
			tmp3 = tmp2;
		}
		tmp2 = tmp2->next;
	}
...
int main()
{
	...
	Task t;
		...
		tmp[strlen(tmp) - 1] = '\0'; // delete '\n'		
        	...
        	burst = atoi(temp);//atoi(strsep(&temp,",")); // There is no "," at last, is it ok so ??? But there is a '\n' which we haven't deleted. 
	...
	printf("turnaround time: 96.25 units\nwaiting time: 75 units\nresponse time:75 units\n");
    	return 0;
}
